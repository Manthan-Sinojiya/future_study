                ...
                for ($i = 1; $i <= $total; $i++) {
                    ...
                    if (!empty($_FILES['image']['tmp_name'][$i])) {
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"][$i], $target_file)) {
                            ...
                            $sql_insert = "INSERT INTO re_quiz (title, text, option1, option2, option3, option4, correct_answer, quiz_id, image_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            $stmt_insert = $conn->prepare($sql_insert);

                            // Bind parameters and execute the query for each question
                            $stmt_insert->bind_param('sssssssss', $title, $text, $option1, $option2, $option3, $option4, $correct_answer, $quiz_id, $image_path);
